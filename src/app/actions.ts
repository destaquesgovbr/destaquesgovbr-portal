...
export const getThemes = withResult(
  async (): Promise<GetThemesResult> => {
    try {
      ...
      // Usar getPrioritizedThemes para selecionar temas baseado no modo configurado
      const { getPrioritizedThemes, calculateThemeScores } = await import('@/lib/prioritization')
