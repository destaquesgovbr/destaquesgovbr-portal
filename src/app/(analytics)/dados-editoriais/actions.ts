...
export const getKpis = withResult(async (range: Interval) => {
  // total
  const totalRes = await typesense
  ...
  // temas ativos
  const temasRes = await typesense
    .collections<ArticleRow>('news')
    .documents()
    .search({
      q: '*',
  ...
  const temasAtivos = (temasRes.grouped_hits ?? []).filter(
    (g) => g.group_key?.[0],
  ).length

  // órgãos ativos
  const orgaosRes = await typesense
    .collections<ArticleRow>('news')
    .documents()
    .search({
      q: '*',
  ...
  const orgaosAtivos = (orgaosRes.grouped_hits ?? []).filter(
    (g) => g.group_key?.[0],
  ).length
...
export const getTopThemes = withResult(
  async (range: Interval, limit: number = 8) => {
...
export const getTopAgencies = withResult(
  async (range: Interval, limit: number = 8) => {
    const res = await typesense
      .collections<ArticleRow>('news')
      .documents()
...
export const getTimelineDaily = withResult(async (range: Interval) => {
  ...
  const daySec = 86400
  for (
    let t = getUnixTime(range.start);
    t < getUnixTime(range.end);
    t += daySec
  ) {
    const dayStart = t
...
export const getThemeComparison = withResult(
  async (current: Interval, previous: Interval) => {
    ...
    const withGrowth = currentThemes.data.map((curr) => {
      ...
      const growth =
        prevCount > 0 ? ((curr.count - prevCount) / prevCount) * 100 : 100
...
export const getAgencyComparison = withResult(
  async (current: Interval, previous: Interval) => {
    ...
    const prevMap = new Map(
      previousAgencies.data.map((a) => [a.agency, a.count]),
    )
